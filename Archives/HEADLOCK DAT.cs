...
namespace Verviewer.Archives
{
    ...
    internal sealed class DatArchiveHandler : IArchiveHandler
    {
        ...
        public Stream OpenEntryStream(OpenedArchive arc, ArchiveEntry entry)
        {
            ...
            return new MemoryStream(block.Data, false);
        }
        ...
        static List<ArchiveEntry> Parse(FileStream fs)
        {
            ...
            var entries =
                TryParseLayout(fs, index, fileSize, dataStart, 8, 4) ??
                TryParseLayout(fs, index, fileSize, dataStart, 12, 8);
        ...
        static Block ReadBlock(Stream s, long offset)
        {
            ...
            byte[] data;
            if (fsize > 0 && checksum != 0) // 这里的判断参考自https://github.com/punk7890/PS2-Visual-Novel-Tool/blob/main/src/scenes/headlock.gd
            {
        ...
        {
            for (int i = 0; i < count; i++) data[offset + i] ^= key;
        }
    }
}